...
void copy(double **recData,double **newData,int valuesPerProcess,int Dimensions,int NumberOfPoints,int numtasks)
{
    int i,j;
    for (i=0;i<valuesPerProcess ;i++)
    {
...
void loadData(double **Data,int id,int valuesPerProcess,int NumberOfPoints,int Dimensions,int numtasks)
{
    double temp;
    int i ,j,n;
...
void kNearestNeighbours(int k,double **distances,int **index,double **Data1,double **Data2,int valuesPerProcess,int Dimensions,int previous,int numtasks,int id,int flag1,int flag2,int NumberOfPoints)
{
    double dis;
    int i,j,m,p;
    int temp=0;
...
int main(int argc, char **argv)
{
    ...
    loadData(Data,id,valuesPerProcess,NumberOfPoints,Dimensions,numtasks); // load data for every process
    ...
    for (i=0;i<valuesPerProcess;i++)
    {
        for (j=0;j<Dimensions;j++)
        {
            if (id == 0)
            {
                MPI_Send(&(Data[i][j]),1,MPI_DOUBLE,next,0,MPI_COMM_WORLD);
                MPI_Recv(&(recData[i][j]),1,MPI_DOUBLE,prev,0,MPI_COMM_WORLD,&status);
            }
            else
            {
                MPI_Recv(&(recData[i][j]),1,MPI_DOUBLE,prev,0,MPI_COMM_WORLD,&status);
    ...
    kNearestNeighbours(k,distances,index,Data,Data,valuesPerProcess,Dimensions,prev,numtasks,id,flag1,flag2,NumberOfPoints);
    flag1 = 1;
    
    for (l=0;l<(numtasks-1);l++)
    {
        
        for (i=0;i<(valuesPerProcess + (NumberOfPoints % numtasks));i++)
        {
            for (j=0;j<Dimensions;j++)
            {
                if (id == 0)
                {
                    MPI_Send(&(recData[i][j]),1,MPI_DOUBLE,next,0,MPI_COMM_WORLD);
                    MPI_Recv(&(newData[i][j]),1,MPI_DOUBLE,prev,0,MPI_COMM_WORLD,&status);
                }
                else
                {
                    MPI_Recv(&(newData[i][j]),1,MPI_DOUBLE,prev,0,MPI_COMM_WORLD,&status);
        ...
        kNearestNeighbours(k,distances,index,Data,recData,valuesPerProcess,Dimensions,prev,numtasks,id,flag1,flag2,NumberOfPoints);
        
        flag2++;
        
        copy(recData,newData,valuesPerProcess,Dimensions,NumberOfPoints,numtasks);
